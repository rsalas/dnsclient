<?php
use DNS\dnsData\Types;
use DNS\dnsData\Exception;
use DNS\dnsResponses\Response;
...
class Protocol
{
    ...
    public function errorHandler($errno = 0, $errstr = null, $errfile = null, $errline = null)
    {
        // If error is suppressed with @, don't throw an exception
        if (error_reporting() === 0) {
            return true; // return true to continue through the others error handlers
        }
        throw new dnsData\Exception('Found '.$errstr.' in line '.$errline.' of '.$errfile, $errno, null);
    }
    ...
    public function query($question, $type = 'A')
    {
        ...
        if ($typeid === false) {
            throw new dnsData\Exception("Invalid Query Type ".$type);
        }
        ...
        if (!$socket = @fsockopen($host, $this->port, $errno, $errstr, $this->timeout)) {
            throw new dnsData\Exception("Failed to open socket to ".$host);
        }
        ...
        if (($this->udp) && ($headersize>=512)) {
            fclose($socket);
            throw new dnsData\Exception("Question too big for UDP (".$headersize." bytes)");
        }
        ...
        if ($this->udp) {// UDP method
            ...
            if (!$this->rawbuffer=fread($socket, 4096)) { // read until the end with UDP
                fclose($socket);
                throw new dnsData\Exception("Failed to write read data buffer");
            }
        } else {// TCP method
            if (!fwrite($socket, $headersizebin)) { // write the socket
                fclose($socket);
                throw new dnsData\Exception("Failed to write question length to TCP socket");
            }

            if (!fwrite($socket, $header, $headersize)) {
                fclose($socket);
                throw new dnsData\Exception("Failed to write question to TCP socket");
            }
            ...
            if (!$this->rawbuffer=fread($socket, $datasize)) {
                fclose($socket);
                throw new dnsData\Exception("Failed to read data buffer");
            }
        }
        fclose($socket);
        ...
        if ($buffersize<12) {
            throw new dnsData\Exception("DNS query return buffer too small");
        }
        ...
        $flags = sprintf("%016b\n", $this->header['flags']);
        $response=new dnsResponses\Response();
        ...
        for ($a=0; $a<$this->header['ancount']; $a++) {
            $response->readRecord($this->rawbuffer, dnsResponses\Response::RESULTTYPE_RESOURCE);
        }
        ...
        for ($a=0; $a<$this->header['nscount']; $a++) {
            $response->readRecord($this->rawbuffer, dnsResponses\Response::RESULTTYPE_NAMESERVER);
        }
        ...
        for ($a=0; $a<$this->header['arcount']; $a++) {
            $response->readRecord($this->rawbuffer, dnsResponses\Response::RESULTTYPE_ADDITIONAL);
        }
        return $response;
    }
    ...
    private function base32encode($input, $padding = true)
    {
        $map = array(
            '0', '1', '2', '3', '4', '5', '6', '7', //  7
            '8', '9', 'a', 'b', 'c', 'd', 'e', 'f', // 15
